...
export async function generateArchive() {
  ...
  const listConversationsRes = await fetch(`${BASE_URL}/backend-api/conversations`, {
    headers: {
  ...
  for (const conversation of listConversationsData.items) {
    ...
    const getConversationRes = await fetch(`${BASE_URL}/backend-api/conversation/${conversationId}`, {
      headers: {
        "Authorization": `Bearer ${accessToken}`,
      }
    });
